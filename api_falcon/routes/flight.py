...
class FlightAllObjects:
    ...
    def on_get(self, request, response, id):
        related_flight = models.Flight.objects.get(id=id)
        all_objects = [x.convert_to_json() for x in models.Object.objects(flight=related_flight)]
        ...
        response.media = all_objects
        response.status = falcon.HTTP_OK if len(response.media) > 0 else falcon.HTTP_NO_CONTENT

    def on_delete(self, request, response, id):
        related_flight = models.Flight.objects.get(id=id)
        all_objects = [x for x in models.Object.objects(flight=related_flight)]
        [x.delete() for x in all_objects]
        response.status = falcon.HTTP_OK
